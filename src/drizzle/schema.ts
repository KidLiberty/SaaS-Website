...
export const ProductTable = pgTable('products', {
  ...
  createdAt,
  updatedAt,
}, table => ({ clerkUserIdIndex: index('products.clerk_user_id_index').on(table.clerkUserId) }))
...
export const productCustomizationRelations = relations(ProductCustomizationTable, ({ one }) => ({
  product: one(ProductTable, { fields: [ProductCustomizationTable.productId], references: [ProductTable.id] })
}))
...
export const productViewRelations = relations(ProductViewTable, ({ one }) => ({
  product: one(ProductTable, { fields: [ProductViewTable.productId], references: [ProductTable.id] }),
  country: one(CountryTable, { fields: [ProductViewTable.countryId], references: [CountryTable.id] })
}))
...
export const countryRelations = relations(CountryTable, ({ many, one }) => ({
  countryGroups: one(CountryGroupTable, { fields: [CountryTable.countryGroupId], references: [CountryGroupTable.id], }),
  productViews: many(ProductViewTable)
}))
...
export const CountryGroupDiscountTable = pgTable('country_group_discounts', {
  ...
  createdAt,
  updatedAt
}, table => ({ pk: primaryKey({ columns: [table.countryGroupId, table.productId] }) }))

export const countryGroupDiscountRelations = relations(CountryGroupDiscountTable, ({ one }) => ({
  product: one(ProductTable, { fields: [CountryGroupDiscountTable.productId], references: [ProductTable.id] }),
  countryGroup: one(CountryGroupTable, { fields: [CountryGroupDiscountTable.countryGroupId], references: [CountryGroupTable.id] })
}))
